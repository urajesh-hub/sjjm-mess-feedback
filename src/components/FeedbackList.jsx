...
const FeedbackList = () => {
  ...
  // Mapping of meal types to Firebase collection names and columns
  const mealTypes = {
    Breakfast: {
      label: 'Breakfast',
    ...
    <div className="feedback-list container">
      ...
      <table className="table table-bordered">
        <thead>
          <tr>
            {selectedMeal === 'All'
              ? Object.keys(mealTypes.Breakfast.columns).map((col) => (
                <th key={col}>{col}</th>
              ))
                    ...
                    <td key={col}>
                      {col === 'date' 
                        ? feedback.date?.seconds 
                          ? new Date(feedback.date.seconds * 1000).toLocaleDateString() 
                          : 'N/A'
