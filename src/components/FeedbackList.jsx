...
const FeedbackList = () => {
  ...
  const downloadExcel = () => {
    ...
    const worksheetData = currentPageData.map(feedback => {
      ...
      mealTypes[selectedMeal]?.columns.forEach((col, index) => {
        // Retrieve value, checking for arrays and converting them to a readable format
        const value = Array.isArray(feedback[col])
          ? feedback[col].join(', ') // Convert array to comma-separated string
          : feedback[col] !== undefined
            ? feedback[col]
            : 'N/A';

        // Map column to custom name and value
  ...
  return (
    <div className="container-fluid">
      <h4 className='text-center' style={{ textTransform: "uppercase" }}>{`${selectedMeal} List`}</h4>

      <div className="mb-3 d-flex btn-group btn-group-sm" >
      ...
      {/* Download Button */}
      <button className="btn btn-success m-3" onClick={downloadExcel}>
        Download as Excel
      </button>
